...
import DisplacementMaterial from "./shaders/DisplacementMaterial"
...
function AnimatedSphere() {
  ...
  const targetMouseRef = useRef(new THREE.Vector2(0.8, 0.5))
  const { size, viewport } = useThree();

  // Handle mouse movement
  useEffect(() => {
    const handleMouseMove = (event) => {
      targetMouseRef.current.x = event.clientX / window.innerWidth * 0.4
      targetMouseRef.current.y = event.clientY / window.innerHeight * 0.4
    }
  ...
  useFrame((state) => {
    ...
    mouseRef.current.x = THREE.MathUtils.lerp(mouseRef.current.x, targetMouseRef.current.x, 0.05)
    mouseRef.current.y = THREE.MathUtils.lerp(mouseRef.current.y, targetMouseRef.current.y, 0.05)

    if (materialRef.current) {
      materialRef.current.time = state.clock.elapsedTime * 0.05
    ...
    if (meshRef.current) {
      ...
      meshRef.current.rotation.x = mouseRef.current.y * 0.3 - 0.5
      meshRef.current.rotation.y = mouseRef.current.x * 0.3 - 0.5
    }
  })

  // Position sphere similar to the original code
  ...
  return (
    <mesh ref={meshRef} position={spherePosition} scale={[33, 33, 33]}>
      <sphereGeometry args={[1.0, 400, 200]} />
...
export default function SpecimenSphere({ isLightMode, themeSetter }) {
  ...
  return (
    <>
      <main className={`w-full h-screen bg-gradient-to-br relative overflow-hidden ${color} ${bgColor} transition-colors duration-150`}>
        ...
        {/* Overlay */}
        <div className={`absolute inset-0 w-dvw h-dvh z-10 backdrop-blur-[0.1px] ${isLightMode ? "light-gradient" : "dark-gradient"}`}></div>

        {/* 3D Canvas */}
        <article className="w-dvw h-dvh fixed inset-0">
          <Canvas
            camera={{ position: [0, 0, 44], fov: 54, }}
            className={`absolute inset-0 transition-opacity duration-[2s] ${opacity}`}
            gl={{
              antialias: false,
              alpha: true,
          ...
          >
            ...
            <directionalLight position={[100, 100, 200]} intensity={2.0} color="white" />
            <AnimatedSphere />
          </Canvas>
        </article>
